...
class loihi():
    ...
    def build(self): 
        ...
        with net: 
            ...
            self.derivativeEnsX = nengo.Ensemble(300, radius=2, dimensions=1, label='derivativeEnsX')
            self.derivativeEnsY = nengo.Ensemble(300, radius=2, dimensions=1, label='derivativeEnsY')
            ...
            self.ux = nengo.Ensemble(300, radius=1, dimensions=1, label='ux')
            self.uy = nengo.Ensemble(300, radius=1, dimensions=1, label='uy')

            self.xDesNode = nengo.Node(xDesired, label='xDesired')
            self.yDesNode = nengo.Node(yDesired, label='yDesired')

            nengo.Connection(self.inputNode[0], self.inputEnsX, transform=1, label='inputNode_to_inputEnsX', synapse=None)
            nengo.Connection(self.inputNode[1], self.inputEnsY, transform=1, label='inputNode_to_inputEnsY', synapse=None)

            nengo.Connection(self.inputNode[0], self.errorEnsX, transform=1, label='xDesired_to_errorEnsX', synapse=None)
            nengo.Connection(self.xDesNode, self.errorEnsX, transform=-1, label='xDesiredNode_to_errorEnsX', synapse=None)
            nengo.Connection(self.inputNode[1], self.errorEnsY, transform=1, label='yDesired_to_errorEnsY', synapse=None)
            nengo.Connection(self.yDesNode, self.errorEnsY, transform=-1, label='yDesiredNode_to_errorEnsY', synapse=None)

            nengo.Connection(self.errorEnsX, self.derivativeEnsX, transform=10, synapse=None, label='inputEnsX_to_derivativeEnsX')
            nengo.Connection(self.errorEnsX, self.delayEnsX, transform=1, synapse=None, label='inputEnsX_to_delayEnsX')
            nengo.Connection(self.delayEnsX, self.derivativeEnsX, transform=-10, synapse=None, label='delayEnsX_to_derivativeEnsX')

            nengo.Connection(self.errorEnsY, self.derivativeEnsY, transform=10, synapse=None, label='inputEnsY_to_derivativeEnsY')
            nengo.Connection(self.errorEnsY, self.delayEnsY, transform=1, synapse=None, label='inputEnsX_to_delayEnsY')
            nengo.Connection(self.delayEnsY, self.derivativeEnsY, transform=-10, synapse=None, label='delayEnsX_to_derivativeEnsY')

            nengo.Connection(self.errorEnsX, self.ux, transform=kpx, label='errorEnsX_to_ux', synapse=None)
            nengo.Connection(self.derivativeEnsX, self.ux, transform=kdx, label='derivativeEnsX_to_ux', synapse=None)
            nengo.Connection(self.errorEnsY, self.uy, transform=kpy, label='errorEnsY_to_uy', synapse=None)
            nengo.Connection(self.derivativeEnsY, self.uy, transform=kdy, label='derivativeEnsY_to_uy', synapse=None)

            nengo.Connection(self.ux, self.outputNode[0], transform=1, label='ux_to_servoX', synapse=nengo.synapses.Alpha(0.1))
            nengo.Connection(self.uy, self.outputNode[1], transform=1, label='uy_to_servoY', synapse=nengo.synapses.Alpha(0.1))
            ...
            self.uxProbe = nengo.Probe(self.ux, synapse=nengo.synapses.Alpha(0.01), label='derivativeProbeX') # 0.05 - 0.1 Alpha is the best
            self.uyProbe = nengo.Probe(self.uy, synapse=nengo.synapses.Alpha(0.01), label='derivativeProbeY')
            # nengo.synapses.Alpha(0.1)
            return net
    def get_probes(self):
        return [self.inputProbeX, self.inputProbeY, self.uxProbe, self.uyProbe]
    ...
    def plot(self): 
        ...
        plt.title('uxProbe')  # Set title
        plt.xlabel('X-Axis Label')  # Set x-axis label
        plt.ylabel('Y-Axis Label')  # Set y-axis label

        plt.subplot(2, 3, 5)
        plt.scatter(self.sim.trange(), self.sim.data[data[3]])
        plt.title('uyProbe')  # Set title
        plt.xlabel('X-Axis Label')  # Set x-axis label
        plt.ylabel('Y-Axis Label')  # Set y-axis label

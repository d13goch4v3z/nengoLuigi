...
class experiment: 
    ...
    def build(self): 
        ...
        with net: 
            ...
            # Connections
            nengo.Connection(self.stepNode, self.stepEns, transform=1, label='stepNode_to_stepEns')
            nengo.Connection(self.sineNode, self.sineEns, transform=1, label='sineNode_to_sineEns')
            # Probes
    ...
    def printInformation(self, sim): 
        print("<---------------Chip to host Receivers--------------->")
        print(sim.sims['loihi'].model.chip2host_receivers)
